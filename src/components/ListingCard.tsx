...
export default function ListingCard({ listing }: { listing: Listing }) {
  ...
  const go = (e: React.MouseEvent, dir: 1 | -1) => {
    e.preventDefault();
    e.stopPropagation();
    setIdx((i) => (i + dir + listing.images.length) % listing.images.length);
  };

  return (
    <Link href={`/rooms/${listing.id}`} className="group block">
      <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-border-soft">
        ...
        >
          <svg
            viewBox="0 0 24 24"
            className={`h-7 w-7 ${isFav ? "fill-rausch" : "fill-black/40"} stroke-white`}
            strokeWidth={2}
          >
          ...
          <>
            ...
            <div className="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1.5">
              {listing.images.map((_, i) => (
                <span
                  key={i}
                  className={`h-1.5 rounded-full transition-all ${i === idx ? "w-1.5 bg-white" : "w-1.5 bg-white/60"}`}
                />
              ))}
            </div>
          </>
      ...
      <div className="mt-2.5">
        <div className="flex items-start justify-between gap-2">
          <h3 className="truncate font-medium text-foreground">{listing.location}</h3>
